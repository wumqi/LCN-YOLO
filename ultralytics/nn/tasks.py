...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
        ...
        elif m in frozenset({Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, Detect_ASFF,
                             v11Detect_ASFFHead}):
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, Segment, Pose, OBB, Detect_ASFF, v11Detect_ASFFHead}:
                m.legacy = legacy
            ...
            args = [*args[1:]]
        # ----------ADD attention------------
        elif m is {CBAM, ECA, CoordAtt, GAM_Attention, MSCAAttention, CAA, PPA, PPAS}:
            c2 = ch[f]
            args = [c2, *args]
        elif m is CBAME:
            c1 = ch[f]
